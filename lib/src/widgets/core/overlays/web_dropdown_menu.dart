...
class _WebSettingsDropdownState extends State<_WebSettingsDropdown> {
          ...
          return MaterialIconButton(
            ...
            onTapDown: (details) async {
              final _settingsMenu = await showMenu<String>(
                context: context,
                items: [
                  if (_podCtr.videoQalityUrls.isNotEmpty)
                    PopupMenuItem(
                      value: 'OUALITY',
                      child: _bottomSheetTiles(
                        title: _podCtr.podPlayerLabels.quality,
                        icon: Icons.video_settings_rounded,
                        subText:_podCtr.playingVideoQuality == -1 ?'Auto' :'${_podCtr.playingVideoQuality}p',
                      ),
                    ),
                  PopupMenuItem(
                    value: 'LOOP',
  ...
  Future<void> _onVimeoQualitySelect(
    ...
    await showMenu(
      context: context,
      items: _podCtr.videoQalityUrls
          .map(
            (e) => PopupMenuItem(
              child: ListTile(
                title: Text(e.quality ==-1 ?'Auto':'${e.quality}p'),
              ),
              onTap: () {
